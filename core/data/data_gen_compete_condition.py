...
class CompeteDataGen(DataLoader):
    ...
    @classmethod
    def initiate_evaluation_dataloader(cls, data_num, n_node_histogram, batch_size=4, max_n_nodes=60, condition_values=None, **kwargs):
        """
        ...
        # Add condition values to the dataset if provided
        if condition_values is not None:
            # Ensure we have the right number of condition values
            if len(condition_values) != data_num:
                raise ValueError(f"Number of condition values ({len(condition_values)}) must match data_num ({data_num})")
            
            # Convert to tensor and add to dataset
            condition_tensor = torch.tensor(condition_values, dtype=torch.float32)  # [data_num]
            ds.data.condition_value = condition_tensor
            
            # Update slices for the new attribute
            ds.slices['condition_value'] = torch.arange(0, data_num + 1, dtype=torch.long)
