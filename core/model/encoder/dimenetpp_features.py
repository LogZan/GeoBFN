...
class Envelope(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        x_pow_p0 = safe_x.pow(p - 1)
        x_pow_p1 = x_pow_p0 * safe_x
        x_pow_p2 = x_pow_p1 * safe_x
        
        out = 1. / safe_x + a * x_pow_p0 + b * x_pow_p1 + c * x_pow_p2
        return out * C
...
class dist_emb(torch.nn.Module):
    ...
    def forward(self, dist):
        ...
        # 应用包络函数并计算嵌入
        env_value = self.envelope(dist)
        sin_value = (self.freq * dist).sin()
        result = env_value * sin_value
...
class angle_emb(torch.nn.Module):
    ...
    def forward(self, dist, angle, idx_kj):
        # 安全处理距离和角度
        dist = torch.clamp(dist, min=1e-8) / self.cutoff
        
        # 处理可能的NaN角度 (例如当三点共线时)
        valid_angle = torch.isfinite(angle)
        safe_angle = torch.where(valid_angle, angle, torch.zeros_like(angle))
        ...
        # 处理角度函数
        cbf_list = []
        for i, func in enumerate(self.sph_funcs):
            cbf_val = func(safe_angle)
            # 无效角度处理
            if i > 0:  # 第一个球谐函数是常数，不受角度影响
                cbf_val = torch.where(valid_angle, cbf_val, torch.zeros_like(cbf_val))
            cbf_list.append(cbf_val)
        
        cbf = torch.stack(cbf_list, dim=1)
